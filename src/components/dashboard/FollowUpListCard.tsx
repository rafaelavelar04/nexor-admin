...
export const FollowUpListCard = ({ title, items, badgeText, badgeClass }: FollowUpListCardProps) => {
  return (
    <Card className="bg-gray-800/50 border-gray-700 text-white col-span-1 lg:col-span-3">
      ...
      <CardContent>
        {items.length === 0 ? (
          ...
          <div className="space-y-2">
            {items.map(item => (
              <Link to={`/admin/${item.type === 'Lead' ? 'leads' : 'opportunities'}/${item.id}`} key={`${item.type}-${item.id}`} className="block p-2 rounded-md hover:bg-gray-700/50 transition-colors">
                <div className="flex justify-between items-center">
                  ...
                  <p className="text-sm text-gray-300">{format(new Date(item.date), 'dd/MM/yyyy', { locale: ptBR })}</p>
                </div>
              </Link>
            ))}
          </div>
