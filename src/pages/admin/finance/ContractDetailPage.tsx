...
import { Loader2, ArrowLeft, DollarSign, Building, Calendar, FileText, PlusCircle, MoreHorizontal } from 'lucide-react';
...
import { Cost, CostFormDialog } from '@/components/finance/CostFormDialog';
...
const costStatusStyles: Record<string, string> = {
  previsto: "bg-yellow-500/20 text-yellow-300 border-yellow-500/30",
...
const ContractDetailPage = () => {
  ...
  const { data, isLoading, isError } = useQuery({
    ...
    queryFn: async () => {
      if (!id) return null;
      const { data, error } = await supabase.rpc('get_contract_details', { contract_id_param: id }).single();
  ...
  const { data: costs, isLoading: isLoadingCosts } = useQuery({
    ...
    queryFn: async () => {
      if (!id) return [];
      const { data, error } = await supabase.from('partner_costs').select('*, partner:partners(nome)').eq('contract_id', id);
      if (error) throw error;
      return data || [];
    },
    enabled: !!id,
  ...
  const deleteCostMutation = useMutation({
    mutationFn: async (costId: string) => {
      const { error } = await supabase.from('partner_costs').delete().eq('id', costId);
      if (error) throw error;
    },
    onSuccess: () => {
      showSuccess("Custo removido com sucesso.");
      queryClient.invalidateQueries({ queryKey: ['contractCosts', id] });
    },
    onError: (error: any) => showError(`Erro ao remover custo: ${error.message}`),
  });
      ...
      <div className="space-y-6">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between">
            ...
            {canManage && <Button size="sm" onClick={() => { setSelectedCost(null); setIsCostFormOpen(true); }}><PlusCircle className="w-4 h-4 mr-2" />Adicionar Custo</Button>}
          </CardHeader>
          <CardContent>
            ...
            <Table>
              <TableHeader><TableRow><TableHead>Parceiro</TableHead><TableHead>Centro de Custo</TableHead><TableHead>Status</TableHead><TableHead className="text-right">Valor</TableHead><TableHead className="w-12"></TableHead></TableRow></TableHeader>
              <TableBody>
                {costs && costs.length > 0 ? costs.map(cost => (
                  <TableRow key={cost.id}>
                    <TableCell>{cost.partner?.nome}</TableCell>
                    <TableCell>{cost.centro_custo}</TableCell>
                    <TableCell><Badge className={costStatusStyles[cost.status]}>{cost.status}</Badge></TableCell>
                    <TableCell className="text-right">{formatCurrency(cost.valor)}</TableCell>
                    <TableCell>
                      <DropdownMenu>
                        <DropdownMenuTrigger asChild><Button variant="ghost" className="h-8 w-8 p-0"><MoreHorizontal className="h-4 w-4" /></Button></DropdownMenuTrigger>
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem onClick={() => { setSelectedCost(cost as Cost); setIsCostFormOpen(true); }}>Editar</DropdownMenuItem>
                          <DropdownMenuItem onClick={() => deleteCostMutation.mutate(cost.id)} className="text-destructive">Excluir</DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
                    </TableCell>
                  </TableRow>
                )) : <TableRow><TableCell colSpan={5} className="text-center text-muted-foreground">Nenhum custo lançado.</TableCell></TableRow>}
              </TableBody>
            </Table>
          </CardContent>
        </Card>
